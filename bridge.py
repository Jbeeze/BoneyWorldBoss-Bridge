...
def _extract_balanced_table(content: str, key_marker: str) -> str | None:
    """Find `key_marker` in `content` and return the substring from the
    next `{` through its matching `}` (inclusive). Returns None if the
    marker isn't present or braces don't balance."""
...
def parse_dbm_stats(path: str, verbose: bool = False) -> dict | None:
    ...
    The structure is nested (top-level scalars + ["bosses"][bossKey] sub-tables),
    so the existing flat field-regex won't work. We strip the bosses sub-block
    out before pulling top-level scalars to avoid key collisions, then parse
    each boss sub-table separately."""
    try:
        with open(path, "r", encoding="utf-8", errors="replace") as f:
            content = f.read()
    except IOError:
    ...
    outer = _extract_balanced_table(content[block_start:], '["dbmStats"]')
    if outer is None:
        return None

    bosses_block = _extract_balanced_table(outer, '["bosses"]')
    ...
    if bosses_block is not None:
        for boss_key in ("kazzak", "doomwalker"):
            sub = _extract_balanced_table(bosses_block, f'["{boss_key}"]')
            if sub is None:
                continue
...
def check_dbm_stats(state: dict, verbose: bool = False) -> None:
    ...
    try:
        if not _runtime_config.get("discordId"):
            if verbose:
...
def parse_char_profile(path: str, verbose: bool = False) -> dict | None:
    ...
    Shape: { timestamp, realm, roles = { [characterName] = "tank"|"healer"|"dps", ... } }

    The roles sub-table is keyed on user-supplied character names (which may
    contain characters outside `\\w`), so we use a permissive [^"]+ pattern
    rather than the addon-key `\\w+` pattern used by the other parsers."""
    try:
        with open(path, "r", encoding="utf-8", errors="replace") as f:
            content = f.read()
    except IOError:
    ...
    outer = _extract_balanced_table(content[block_start:], '["charProfile"]')
    if outer is None:
        return None

    roles_block = _extract_balanced_table(outer, '["roles"]')
    top_only = outer.replace(roles_block, "{}") if roles_block else outer

    # Top-level scalars (timestamp, realm) — keys here are addon-controlled
    timestamp = None
    realm = ""
    ...
    if roles_block is not None:
        # Permissive char-name match; role value is one of three known words
...
def main_loop() -> None:
    ...
    # Do initial DBM stats snapshot check (verbose)
    print("[DBM_STATS] Checking for DBM stats snapshot...")
    check_dbm_stats(state, verbose=True)

    # Do initial character profile snapshot check (verbose)
